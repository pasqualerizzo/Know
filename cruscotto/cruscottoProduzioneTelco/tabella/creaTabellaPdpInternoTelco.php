...
foreach ($mandato as $idMandato) {
    switch ($idMandato) {
        case "Tim":
            $queryCrm = "SELECT "
                    . "statoPda, fasePDA, COUNT(statoPDA) ,SUM(if((dataVendita<='$dataMaggiore'and dataVendita>='$dataMinore'),1,0)) "
                    . "FROM "
                    . "`tim`  "
                    . "inner join aggiuntaTim ON  tim.id=aggiuntaTim.id "
                    . "where mese='$mese' and statoPda<>'Bozza' and statoPda<>'Annullata' and statoPda<>'Pratica doppia' and statoPda<>'In attesa Sblocco'"
                       . $querySede
                    . " group by statoPda";
//echo $queryCrm;
