...
//echo $queryCrm;
    ...
    case "Union":
        ...
        $queryCrm = "SELECT "
                ...
                . " sum(if(metodoInvio='Cartaceo',pezzoLordo,0)),sum(if(metodoInvio='Bollettaweb',pezzoLordo,0)), "
                . " sum(if(comodity='Luce',pezzoLordo,0)),sum(if(comodity='Gas',pezzoLordo,0)),sum(if(comodity='Dual',pezzoLordo,0)),sum(if(comodity='Polizza',pezzoLordo,0)),"
                . " sum(if(metodoPagamento='Bolletta',if(fasePDA='OK',pezzoLordo,0),0)),"
                . " sum(if(metodoPagamento='RID',if(fasePDA='OK',pezzoLordo,0),0)),"
                . " sum(if(metodoInvio='Cartaceo',if(fasePDA='OK',pezzoLordo,0),0)),"
                . " sum(if(metodoInvio='Bollettaweb',if(fasePDA='OK',pezzoLordo,0),0)), "
                . " sum(if(dataSwitchOutLuce<>'0000-00-00'and dataSwitchOutGas='0000-00-00',1,0)), "
...
//
//$querylavorativi ="WITH giorni_mese AS (
//    SELECT DATE_ADD(DATE_SUB(CURRENT_DATE, INTERVAL DAYOFMONTH(CURRENT_DATE) - 1 DAY), INTERVAL seq.seq DAY) AS giorno
//    FROM seq_0_to_31 seq
//    WHERE MONTH(DATE_ADD(DATE_SUB(CURRENT_DATE, INTERVAL DAYOFMONTH(CURRENT_DATE) - 1 DAY), INTERVAL seq.seq DAY)) = MONTH(CURRENT_DATE)
//),
//giorni_lavorati AS (
//    SELECT 
//        COUNT(DISTINCT giorno) AS giorni_lavorati_effettivi,
//        SUM(
//            CASE 
//                WHEN DAYOFWEEK(giorno) BETWEEN 2 AND 6 THEN 1
//                WHEN DAYOFWEEK(giorno) = 7 THEN 0.5
//                ELSE 0
//            END
//        ) AS giorni_lavorati_ponderati
//    FROM `stringheTotale`
//    WHERE giorno BETWEEN (SELECT MIN(giorno) FROM giorni_mese) AND (CURRENT_DATE - INTERVAL 1 DAY)
//),
//giorni_lavorabili AS (
//    SELECT 
//        SUM(
//            CASE 
//                WHEN DAYOFWEEK(giorno) BETWEEN 2 AND 6 THEN 1
...
//                ELSE 0
//            END
//        ) AS giorni_lavorabili_previsti
//    FROM giorni_mese
//)
//SELECT 
//    CAST(giorni_lavorabili_previsti AS DECIMAL(10,2)) AS giorni_lavorabili_previsti,
//    CAST(giorni_lavorati_effettivi AS DECIMAL(10,2)) AS giorni_lavorati_effettivi
//FROM giorni_lavorati, giorni_lavorabili";
//
//
////echo $querylavorativi;
//
//$risultatoCrm = $conn19->query($querylavorativi);
//while ($rigaCRM = $risultatoCrm->fetch_array()) {
//    //echo var_dump($rigaCRM);
//    $giornilavorativi = $rigaCRM[0];
//    $giornilavorati = $rigaCRM[1];
//   
//}
//echo $giornilavorativi;
//echo "<br>";
//echo $giornilavorati;
//
...
while ($rigaCRM = $risultatoCrm->fetch_array()) {
    ...
    $queryGroupMandato = "SELECT sum(numero)/3600 as ore "
            ...
            . " and sede='$sede' and idMandato='$idMandato' and mandato.tipoCampagna='$descrizioneMandato' "
            . "group by sede, idMandato, mandato.tipoCampagna ";
//    echo $queryGroupMandato;
    ...
    $deltaSwo3 = $rigaCRM[34];
    $deltaSwo6 = $rigaCRM[35];
    $deltaSwo9 = $rigaCRM[36];
    $deltaSwoEver = $rigaCRM[37];
...
$resaPezzoOkPf = ($orepaf == 0) ? 0 : round($pezzoOkPaf / $orepaf, 2);
    ...
    if ($sedePrecedente == "" || $sedePrecedente == $sede) {
        ...
        $orepafSedeParziale += round($orepaf, 2);
    ...
    } elseif ($sedePrecedente != $sede) {

        $queryGroupMandato = "SELECT sum(numero)/3600 as ore "
                . "FROM `stringheTotale` inner join mandato on stringheTotale.mandato=mandato.descrizione "
                . "where giorno>='$dataMinore' and giorno<='$dataMaggiore' and livello<=6  "
                . " and upper(sede)=upper('$sedePrecedente') and idMandato='$idMandato'  "
                . "group by sede ";
//        echo $queryGroupMandato;
        //echo "<br>";
        $risultaOre = $conn19->query($queryGroupMandato);
        ...
        $resaPezzopafSede = ($orepafSede == 0) ? 0 : round($pezzoOkPafSede / $orepafSede, 2);
        ...
        $percentualeBollettinoSede = (($pezzoBollettinoSede + $pezzoRidSede) == 0) ? 0 : round((($pezzoBollettinoSede / ($pezzoBollettinoSede + $pezzoRidSede)) * 100), 2);
        $percentualeInvioSede = (($pezzoCartaceoSede + $pezzoMailSede) == 0) ? 0 : round(($pezzoMailSede / ($pezzoCartaceoSede + $pezzoMailSede)) * 100, 2);

        $percentualeBollettinoOKSede = (($pezzoBollettinoOKSede + $pezzoRidOKSede) == 0) ? 0 : round((($pezzoBollettinoOKSede / ($pezzoBollettinoOKSede + $pezzoRidOKSede)) * 100), 2);
        $percentualeInvioOKSede = (($pezzoCartaceoOKSede + $pezzoMailOKSede) == 0) ? 0 : round(($pezzoMailOKSede / ($pezzoCartaceoOKSede + $pezzoMailOKSede)) * 100, 2);
        $percentualeDataSwoSede = ($pezzoPostOkSede == 0) ? 0 : (round(($dataSwoSede / $pezzoPostOkSede) * 100, 2));

        $html .= "<tr style = 'background-color: orange;border: 2px solid lightslategray'>";
        $html .= "<td colspan = '3'>$sedePrecedente</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$pezzoLordoSede</td>";
        $html .= "<td>$pesoLordoSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$pezzoOkSede</td>";
        $html .= "<td>$pesoPdaOkSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$pezzoKoSede</td>";
        $html .= "<td>$pesoPdaKoSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$pezzoBklSede</td>";
        $html .= "<td>$pesoPdaBklSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$pezzoBklpSede</td>";
        $html .= "<td>$pesoPdaBklpSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$oreSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$resaPezzoLordoSede</td>";
        $html .= "<td>$resaValoreLordoSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$resaPezzoOkSede</td>";
        $html .= "<td>$resaValoreOkSede</td>";
        $html .= "<td></td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$pezzoBollettinoSede</td>";
        $html .= "<td>$pezzoRidSede</td>";
        $html .= "<td>$percentualeBollettinoSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$pezzoBollettinoOKSede</td>";
        $html .= "<td>$pezzoRidOKSede</td>";
        $html .= "<td>$percentualeBollettinoOKSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$pezzoCartaceoSede</td>";
        $html .= "<td>$pezzoMailSede</td>";
        $html .= "<td>$percentualeInvioSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$orepafSede</td>";
        $html .= "<td>$pezzoOkPafSede</td>";
        $html .= "<td>$resaPezzopafSede</td>";


        $html .= "<td style = 'border-left: 2px solid lightslategray'>$pezzoLuceSede</td>";
        $html .= "<td>$pezzoGasSede</td>";
        $html .= "<td>$pezzoDualSede</td>";
        $html .= "<td>$pezzoPolizzaSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$pezzoPostOkSede</td>";
        $html .= "<td>$resaPostOKSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$pezzoPostKoSede</td>";
        $html .= "<td>$resaPostKOSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$pezzoPostBklSede</td>";
        $html .= "<td>$resaPostBklSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$dataSwoSede</td>";
        $html .= "<td>$percentualeDataSwoSede</td>";

        $html .= "<td style = 'border-left: 2px solid lightslategray'>$deltaSwo1Sede</td>";
        $html .= "<td>$deltaSwo3Sede</td>";
        $html .= "<td>$deltaSwo6Sede</td>";
        $html .= "<td>$deltaSwo9Sede</td>";
        $html .= "<td>$deltaSwoEverSede</td>";
...
$queryGroupMandato = "SELECT sum(numero)/3600 as ore "
        . "FROM `stringheTotale` inner join mandato on stringheTotale.mandato=mandato.descrizione "
        . "where giorno>='$dataMinore' and giorno<='$dataMaggiore' and livello<=6  "
        . " and upper(sede)=upper('$sede') and idMandato='$idMandato'  "
        . "group by sede ";
//echo $queryGroupMandato;
$risultaOre = $conn19->query($queryGroupMandato);
...
$resaPezzoOkPafSede = ($orepafSede == 0) ? 0 : round($pezzoOkPafSede / $orepafSede, 2);
$resaPezzoLordoSede = ($oreSede == 0) ? 0 : round($pezzoLordoSede / $oreSede, 2);
$resaValoreLordoSede = ($oreSede == 0) ? 0 : round($pesoLordoSede / $oreSede, 2);
$resaPezzoOkSede = ($oreSede == 0) ? 0 : round($pezzoOkSede / $oreSede, 2);
$resaValoreOkSede = ($oreSede == 0) ? 0 : round($pesoPdaOkSede / $oreSede, 2);
