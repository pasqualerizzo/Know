...
$queryPleniPezzi ="SELECT `mandato` as mandatolo,
       SUM(inserito) as inserito_totale,
       SUM(ok_firma) as ok_firma_totale
FROM (
    SELECT `mandato`, 
           COUNT(`statoPDA`) as inserito, 
           0 as ok_firma
    FROM `plenitude`
    WHERE `statoPDA` NOT IN ('Annullata', 'Bozza', 'Pratica Doppia', 'In attesa Sblocco')
    ...
    SELECT `mandato`, 
           0 as inserito, 
           COUNT(`statoPDA`) as ok_firma
    FROM `plenitude`
    WHERE `statoPDA` = 'Ok Firma'
    and comodity<>'Polizza'
      AND MONTH(`data`) = MONTH(CURRENT_DATE())
      AND YEAR(`data`) = YEAR(CURRENT_DATE())
    GROUP BY `mandato`
...
$queryPleniOre = "SELECT 
    ROUND(SUM(st.ore_mandato_sum), 2) as totale_ore_mandato

FROM (
    SELECT 
        `mandato`,
        SUM(`numero`)/3600 as ore_mandato_sum,
        SUM(`numero`) as ore_mandato
    FROM `stringheTotale`
    WHERE `mandato` IN ('Plenitude', 'Plenitude Valore', 'Plenitude Polizze', 'Lead Generico', 'Lead Inbound', 'Comparatore', 'Risparmiami')
    AND `sede` NOT IN ('bo', 'tl', ' ')
      AND MONTH(`giorno`) = MONTH(CURRENT_DATE())
      AND YEAR(`giorno`) = YEAR(CURRENT_DATE())
    GROUP BY `mandato`
) as st";

$resapleni = $queryPleniPezzi / $queryPleniOre ;

       

$queryPleniSedi = "SELECT 
    `mandato` AS mandato,
    `sede`,
    SUM(`inserito`) AS inserito_totale,
    SUM(`ok_firma`) AS ok_firma_totale
FROM (
    SELECT 
        `mandato`, 
        `sede`,
        SUM(CASE WHEN `comodity` = 'DUAL' THEN 2 ELSE 1 END) AS inserito, 
        0 AS ok_firma
    FROM 
        `plenitude`
    WHERE 
        `statoPDA` NOT IN ('Annullata', 'Bozza', 'Pratica Doppia', 'In attesa Sblocco')
        AND `comodity` <> 'Polizza'
        AND MONTH(`data`) = MONTH(CURRENT_DATE())
        AND YEAR(`data`) = YEAR(CURRENT_DATE())
    GROUP BY 
    ...
    SELECT 
        ...
        `sede`,
        0 AS inserito, 
        SUM(CASE WHEN `comodity` = 'DUAL' THEN 2 ELSE 1 END) AS ok_firma
    FROM 
        `plenitude`
    WHERE 
        `statoPDA` = 'Ok Firma'
...
$resaplenisedi =  $queryPleniSedi / $queryPleniSediOre ; 



        $querypezziIren  = "SELECT `mandato` as mandato
          ,
       SUM(inserito) as inserito_totale,
       SUM(ok_firma) as ok_firma_totale
FROM (
...
FROM (
    SELECT 
        ...
        SUM(`numero`) as ore_mandato
    FROM `stringheTotale`
    WHERE `mandato` IN ('Iren', 'Iren Valore')
      AND MONTH(`giorno`) = MONTH(CURRENT_DATE())
      AND YEAR(`giorno`) = YEAR(CURRENT_DATE())
    GROUP BY `mandato`
) as st";
        ...
        $resaIren = $querypezziIren / $queryIrenOre;
        
        
        $queryIrenSedi = "SELECT 
    `mandato` AS mandato,
    `sede`,
    SUM(`inserito`) AS inserito_totale,
    SUM(`ok_firma`) AS ok_firma_totale
...
GROUP BY 
    `sede`";
        ...
        $resaIrenSedi =  $queryIrenSedi / $queryIrenSediOre;
        
                
        $queryinseritiVivigas = "SELECT 
    `mandato` AS mandato,
    SUM(`inserito`) AS inserito_totale,
    SUM(`ok_firma`) AS ok_firma_totale
FROM (
...
FROM (
    SELECT 
        ...
        SUM(`numero`) as ore_mandato
    FROM `stringheTotale`
    WHERE `mandato` IN ('Vivigas Energia Retention', 'Vivigas Energia' , 'Vivigas Energia Pro' , 'Vivi Inbound')
    AND `sede` <> 'tl'
      AND MONTH(`giorno`) = MONTH(CURRENT_DATE())
      AND YEAR(`giorno`) = YEAR(CURRENT_DATE())
    GROUP BY `mandato`
        ...
        $resavivi = $queryinseritiVivigas / $queryVivigasOre;
        
        
        $queryVivigasSedi ="SELECT 
    `mandato` AS mandato,
    `sede` AS sede,
    SUM(`inserito`) AS inserito_totale,
    SUM(`ok_firma`) AS ok_firma_totale
...
FROM (
    ...
    FROM 
        `stringheTotale`
    WHERE 
        `mandato` IN ('Vivigas Energia Retention', 'Vivigas Energia', 'Vivigas Energia Pro', 'Vivi Inbound')
        AND MONTH(`giorno`) = MONTH(CURRENT_DATE())
        AND YEAR(`giorno`) = YEAR(CURRENT_DATE())
    and `sede` <>'tl'
    GROUP BY 
