...
$query1= 'SELECT SUM(lavorati) as "somma_lavorati" from (
     SELECT count(plenitudeid) as "lavorati", statopda AS STATO_PDA FROM `vtiger_plenitude`  WHERE datasottoscrizionecontratto = CURRENT_DATE and (statopda = "Ok firma"  and statopda = "Ok Definitivo")  and ( codicecampagna <> "SPN_LEAD") and ( commodity <> "Polizza") GROUP BY statopda) as t';
...
$query2= 'SELECT SUM(in_bkl) as "somma_bkl" 
FROM (
    SELECT count(plenitudeid) as in_bkl, statopda AS STATO_PDA 
    ...
    WHERE datasottoscrizionecontratto = CURRENT_DATE 
      AND (
            ...
            OR (
                (statopda = "Acquisito" 
                OR statopda = "da firmare" 
                OR statopda = "Ok Controllo dati" 
                OR statopda = "Recuperato Dati")
                OR statopda = "Recupero Recall")
