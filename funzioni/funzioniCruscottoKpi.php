...
function recuperoOre($_conn, $_dataMaggiore, $_dataMinore) {
    ...
    $queryOre = "SELECT "
            ...
            . " territory AS citta, "
            . " operatore.user_group AS userGroup, "
            ...
            . "  SUM(CASE WHEN v.campaign_id IN ('SPN_INB', 'VDF_TLCO') THEN wait_sec ELSE 0 END) / 3600 AS 'oreSPN_Dry', "
            ...
            . " SUM(CASE WHEN v.campaign_id = 'SPN_INB' OR v.campaign_id = 'VDF_TLCO' THEN (talk_sec) ELSE 0 END) / 3600 AS 'oreSPN_Tlk' "
...
//    echo $queryOre;
    ...
    while (($rigaOre = $risultatoOre->fetch_array())) {
        ...
        $oreTlk = round($rigaOre['oreSPN_Tlk'], 2);
        $oreDry = round($rigaOre['oreSPN_Dry'], 2);
        $sede = $rigaOre['citta'];
        $usrgrp = $rigaOre['userGroup'];        
        if (!isset($livello)) {
            $livello = $rigaOre['livello'];
            
        }
        $risposta[$operatore] = [$oreIN, $oreOut, $oreTlk, $oreDry, $sede, $usrgrp, $livello, ];
    }
    return $risposta;
}
...
function calcoloDatiRiferimento($_conn, $_dataMaggiore, $_dataMinore) {
    $queryRiferimento = "SELECT"
            . " count(idSponsorizzata) as Lead, "
            . " (SUM(`pleniOk`) + SUM(`vodaOk`) + SUM(`viviOk`) + SUM(`irenOk`)+ SUM(`uniOk`)+SUM(enelOk)) AS OK_CP"
            . " FROM "
...
//echo $queryRiferimento;
    ...
    if (($rigaRiferimento = $risultatoRiferimento->fetch_array())) {
        ...
        $riferimento =[$leadRiferimento,$okRiferimerimento] ;
...
function recuperoVivigas($_conn, $_dataMaggiore, $_dataMinore) {
    $risposta = [];
    ...
    $query = "SELECT "
            ...
            . " where "
            . " idGestioneLead like 'G%' "
            . " and vivigas.data<='$_dataMaggiore' "
            . " and vivigas.data>='$_dataMinore' "
            . " group by "
            . " creatoDa";
...
function recuperoIren($_conn, $_dataMaggiore, $_dataMinore) {
    $risposta = [];
    ...
    $query = "SELECT "
            ...
            . " where "
            . " idGestioneLead like 'G%' "
            . " and iren.data<='$_dataMaggiore' "
            . " and iren.data>='$_dataMinore' "
            . " group by "
            . " creatoDa";
    ...
    while (($riga = $risultato->fetch_array())) {
        ...
        $operatore = $riga[4];
        $risposta[$operatore] = [$ok, $ko, $tot, $convertito,0];
    }
    return $risposta;
}

function recuperoUnion($_conn, $_dataMaggiore, $_dataMinore) {
    $risposta = [];
    ...
    $query = "SELECT "
            ...
            . " where "
            . " idGestioneLead like 'G%' "
            . " and union.data<='$_dataMaggiore' "
            . " and union.data>='$_dataMinore' "
            . " group by "
            . " creatoDa";
...
function recuperoVivigasOut($_conn, $_dataMaggiore, $_dataMinore) {
    $risposta = [];
    $query = "SELECT 
        SUM(IF(aggiuntaVivigas.fasePDA = 'OK', pezzoLordo, 0)) AS OK,
        SUM(IF(aggiuntaVivigas.fasePDA = 'KO', pezzoLordo, 0)) AS KO,
...
function recuperoUnionOut($_conn, $_dataMaggiore, $_dataMinore) {
    $risposta = [];
    $query = " SELECT 
        SUM(IF(aggiuntaUnion.fasePDA = 'OK', 1, 0)) AS OK,
        SUM(IF(aggiuntaUnion.fasePDA = 'KO', 1, 0)) AS KO,
...
function recuperoEnelOut($_conn, $_dataMaggiore, $_dataMinore) {
    $risposta = [];
    $query = "SELECT "
            . " SUM(IF(aggiuntaEnel.fasePDA='OK',1,0)) as 'OK', "
            . " SUM(IF(aggiuntaEnel.fasePDA='KO',1,0)) as 'KO',  "
...
function recuperoIrenOut($_conn, $_dataMaggioreOre, $_dataMinoreOre) {
    $risposta = [];
    $query = "SELECT "
            . " SUM(IF(aggiuntaIren.fasePDA='OK',1,0)) as 'OK', "
            . " SUM(IF(aggiuntaIren.fasePDA='KO',1,0)) as 'KO',  "
...
function elencoOperatoreBMP($_conn) {
    $operatore = [];
    $queryOperatori = "SELECT "
            . " full_name as operatore, "
            . " user_level AS livello, "
            ...
            . " "
            . " FROM vicidial_users AS v "
            . " WHERE user_group = 'INB_BMP'  and active='Y' and full_name not like 'A_OP_%' "
            . " GROUP BY full_name "
            . " order by territory";
    $risultatoOperatori = $_conn->query($queryOperatori);
    while ($riga = $risultatoOperatori->fetch_array(MYSQLI_ASSOC)) {
...
function recuperoVivigasData($_conn, $_dataMaggiore, $_dataMinore) {
    $risposta = [];
    $query = "SELECT 
        SUM(IF(aggiuntaVivigas.fasePDA = 'OK', pezzoLordo, 0)) AS OK,
        SUM(IF(aggiuntaVivigas.fasePDA = 'KO', pezzoLordo, 0)) AS KO,
...
function recuperoUnionData($_conn, $_dataMaggiore, $_dataMinore) {
    $risposta = [];
    $query = " SELECT 
        SUM(IF(aggiuntaUnion.fasePDA = 'OK', 1, 0)) AS OK,
        SUM(IF(aggiuntaUnion.fasePDA = 'KO', 1, 0)) AS KO,
...
function recuperoEnelData($_conn, $_dataMaggiore, $_dataMinore) {
    $risposta = [];
    $query = "SELECT "
            . " SUM(IF(aggiuntaEnel.fasePDA='OK',1,0)) as 'OK', "
            . " SUM(IF(aggiuntaEnel.fasePDA='KO',1,0)) as 'KO',  "
...
function recuperoIrenData($_conn, $_dataMaggioreOre, $_dataMinoreOre) {
    $risposta = [];
    $query = "SELECT "
            . " SUM(IF(aggiuntaIren.fasePDA='OK',1,0)) as 'OK', "
            . " SUM(IF(aggiuntaIren.fasePDA='KO',1,0)) as 'KO',  "
...
function recuperoTim($_conn, $_dataMaggiore, $_dataMinore) {
    $risposta = [];

    $queryTim = "SELECT "
            . " SUM(IF(aggiuntaTim.fasePDA='OK',1,0)) as 'OK', "
            . " SUM(IF(aggiuntaTim.fasePDA='KO',1,0)) as 'KO',  "
            . " count(*) as 'Tot',"
            . " sum(convertito) as 'Convertito', "
            . " sum(if(fasePost='OK',if(fasePDA='OK',pezzoLordo,0),0)) as 'OKPV',"
            . " creatoDa as 'operatore'"
    ...
    while (($riga = $risultatoTim->fetch_array())) {
        $ok = $riga[0];
        $ko = $riga[1];
        $tot = $riga[2];
        $convertito = $riga[3];
...
function recuperoTimData($_conn, $_dataMaggiore, $_dataMinore) {
    $risposta = [];
    ...
    $query = "SELECT "
            ...
            . " where "
            . " idGestioneLead  like 'G%' "
            . " and dataVendita  <='$_dataMaggiore' "
            . " and dataVendita  >='$_dataMinore' "
            . " group by "
            . " creatoDa";
...
function recuperoTimOut($_conn, $_dataMaggiore, $_dataMinore) {
    $risposta = [];
