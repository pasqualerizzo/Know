...
$queryRicerca = "SELECT "
        ...
        . "date_format(unioncf.cf_4508,'%d-%m-%Y') as 'data', "
        . "unioncf.cf_4510 as 'comodity', "
        . "unioncf.cf_4512 as 'Mercato', "
        . "unioncf.cf_4514 as 'Sede', "
        . "unioncf.cf_4614 as 'Metodo Pagamento', "
        . "unioncf.cf_4558 as 'Metodo Invio', "
        . "unioncf.cf_4628 as 'Stato PDA', "
        . "unioncf.cf_4653 as 'Stato Luce', "
        . "unioncf.cf_4651 as 'Stato Gas', "
        . "entity.createdtime AS 'dataCreazione', "
        . "unioncf.cf_4516 as 'Codice Campagna', "
        . "unioncf.unionid AS 'pratica', "
        . "unioncf.cf_4634 AS 'codicePlicoLuce', "
        . "unioncf.cf_4632 AS 'codicePlicoGas', "
        . "unioncf.cf_4518 AS 'tipo acquisizione', "
        . "unioncf.cf_4697 AS 'id gestione lead', "
        . "unioncf.cf_4691 AS 'id leadId' "
        . "FROM "
        . "vtiger_unioncf as unioncf "
        . "inner join vtiger_union as un on unioncf.unionid=un.unionid "
        . "inner join vtiger_crmentity as entity on un.unionid=entity.crmid "
        . "inner join vtiger_users as operatore on entity.smownerid=operatore.id ";
//        . "WHERE "
//        . "irencf.cf_4276 >'2024-03-01'";
 ...
 */ else {
    $queryTruncate = "TRUNCATE TABLE `union`";
    $conn19->query($queryTruncate);
    $queryTruncate2 = "TRUNCATE TABLE `aggiuntaUnion`";
    ...
    while ($riga = $risultato->fetch_array()) {
         ...
         */
        $queryStatoPda = "SELECT * FROM `unionStatoPDA` where descrizione='$statoPDA'";
        $risultatoStatoPda = $conn19->query($queryStatoPda);
        $conteggioStatoPda = $risultatoStatoPda->num_rows;
        if ($conteggioStatoPda == 0) {
            $queryInserimentoStatoPda = "INSERT INTO `unionStatoPDA`(`descrizione`) VALUES ('$statoPDA')";
         ...
         */
        $queryStatoLuce = "SELECT * FROM `unionStatoLuce` where descrizione='$statoLuce'";
        $risultatoStatoLuce = $conn19->query($queryStatoLuce);
        $conteggioStatoLuce = $risultatoStatoLuce->num_rows;
        if ($conteggioStatoLuce == 0) {
            $queryInserimentoStatoLuce = "INSERT INTO `unionStatoLuce`( `descrizione`) VALUES ('$statoLuce')";
         ...
         */
        $queryStatoGas = "SELECT * FROM `unionStatoGas` where descrizione='$statoGas'";
        //echo $queryStatoGas;
        ...
        if ($conteggioStatoGas == 0) {
            $queryInserimentoStatoGas = "INSERT INTO `unionStatoGas`( `descrizione`) VALUES ('$statoGas')";
...
//
        $queryCampagna = "SELECT * FROM `unionCampagna` where nome='$codiceCampagna'";
        //echo $queryStatoGas;
        $risultatoCampagna = $conn19->query($queryCampagna);
        $conteggioCampagna = $risultatoCampagna->num_rows;
        //echo $conteggioStatoGas;
        if ($conteggioCampagna == 0) {
            $queryInserimentoCampagna = "INSERT INTO `unionCampagna`( `nome`) VALUES ('$codiceCampagna')";
            //echo $queryInserimentoStatoGas;
        ...
        $queryInserimento = "INSERT INTO `union`"
...
//            echo "----".$ore;
         ...
         */
        if ($data < "2024-03-01") {
            $queryPesoComodity = "SELECT peso FROM `unionPesiComoditi` WHERE tipoCampagna='$tipoCampagna' AND valore='$comodity' AND dataInizioValidita='$mese'";
            ...
            $queryPesoComodity = "SELECT peso FROM `unionPesiComoditi` WHERE tipoCampagna='$tipoCampagna' AND valore='$comodity' AND dataInizioValidita='$mese' AND tipoAcquisizione='$tipoAcquisizione'";
        ...
        $queryPesoInvio = "SELECT peso FROM `unionPesiInvio` WHERE tipoCampagna='$tipoCampagna' AND valore='$metodoInvio' AND dataInizioValidita='$mese'";
        ...
        $queryPesoPagamento = "SELECT peso FROM `unionPesiMetodoPagamento` WHERE tipoCampagna='$tipoCampagna' AND valore='$metodoPagamento' AND dataInizioValidita='$mese'";
        ...
        $tipoSanataBo = 0;
        if ($sanataBo == "SI") {
            $querySanataBo = "SELECT peso,tipoDetrazione FROM `unionPesiSanata` WHERE tipoCampagna='$tipoCampagna' AND valore='$sanataBo' AND dataInizioValidita='$mese' ";
            //echo $querySanataBo;
...
//
        $queryInserimentoSecondario = "INSERT INTO `aggiuntaUnion"
