 ...
 */ else {
    ...
    while ($riga = $risultato->fetch_array()) {
        ...
        $queryInserimento = "INSERT INTO `vivigas`"
                ...
                . " VALUES ('$user','$data','$comodity','$mercato','$sede','$metodoPagamento','$statoPDA','$statoLuce','$statoGas','$dataImport','vivigas','$idStatoPda','$idStatoLuce','$idStatoGas','$winback','$idMandato','$dataCreazione','$idCampagna','$codiceCampagna','$metodoInvio','$pratica','$sanataBo')";
        ...
        $pesoSanataBo = 0;
        if ($sanataBo == "SI" && $fasePDA == "OK") {
            $querySanataBo = "SELECT peso FROM `vivigasPesiSanata` WHERE tipoCampagna='$tipoCampagna' AND valore='$sanataBo' AND dataInizioValidita='$mese' ";
            //echo $querySanataBo;
...
//                echo "<br>";
        ...
        $pesoTotalePagato = 0;
        if ($fasePDA == "OK") {
            $pesoTotalePagato = $pesoTotaleNetto-$pesoSanataBo;
        }
        ...
        $queryInserimentoSecondario = "INSERT INTO `aggiuntaVivigas`(`id`, `tipoCampagna`, `pesoComodity`, `pesoInvio`, `pesoMPagamento`, `totalePesoLordo`, `faseLuce`, `faseGas`, `totalePesoNetto`,mese,fasePDA,pesoTotalePagato) "
                . "VALUES ('$indiceContratto','$tipoCampagna','$pesoComodity','$pesoInvio','$pesoPagamento','$pesoTotaleLordo','$faseLuce','$faseGas','$pesoTotaleNetto','$mese','$fasePDA','$pesoTotalePagato')";
