 ...
 * @return type
 */
function idStatoPdaEnelIn($conn19, $_statoPda) {
    $queryStatoPda = "SELECT * FROM `enelInStatoPDA` where descrizione='$_statoPda'";
    try {
 ...
 * @return type
 */
function idStatoLuceEnelIn($conn19, $_statoLuce) {
    try {
        $queryStatoLuce = "SELECT * FROM `enelInStatoLuce` where descrizione='$_statoLuce'";
        $risultatoStatoLuce = $conn19->query($queryStatoLuce);
        $conteggioStatoLuce = $risultatoStatoLuce->num_rows;
        if ($conteggioStatoLuce == 0) {
            $queryInserimentoStatoLuce = "INSERT INTO `enelInStatoLuce`( `descrizione`) VALUES ('$_statoLuce')";
 ...
 * @return type
 */
function idStatoGasEnelIn($conn19, $_statoGas) {
    try {
        $queryStatoGas = "SELECT * FROM `enelInStatoGas` where descrizione='$_statoGas'";
        $risultatoStatoGas = $_conn->query($queryStatoGas);
        $conteggioStatoGas = $risultatoStatoGas->num_rows;
        if ($conteggioStatoGas == 0) {
            $queryInserimentoStatoGas = "INSERT INTO `enelInStatoGas`( `descrizione`) VALUES ('$_statoGas')";
