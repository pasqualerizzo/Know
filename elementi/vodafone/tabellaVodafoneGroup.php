...
$queryMese = "SELECT mese FROM aggiuntaVodafone group by mese ORDER by mese DESC";
...
$query = "SELECT creatoDa,round(sum(pesoTotaleLordo),5) as 'Peso Lordo' ,round(sum(pesoTotaleNetto),5)as 'Peso Netto' ,round(sum(pesoPagato),5)as 'Peso Pagato', round(sum(pesoFormazione),5)as 'Peso Formazione' FROM `vodafone` inner join aggiuntaVodafone on vodafone.id=aggiuntaVodafone.id where mese='$meseSelezionato' group by creatoDa";
...
<body>
<header>
    <h1 class="titolo">Vodafone:<br>per Operatore</h1>
</header>
<div>
...
<script>
    ...
    function redirigi(sel) {
        ...
        window.location.href = "tabellaVodafoneGroup.php?meseSelezionato=" + data;
    ...
    function scaricaCSV() {
        ...
        window.location.href = "exportTabellaVodafoneGroup.php?meseSelezionato=" + valore;
    }
</script>
</html>
