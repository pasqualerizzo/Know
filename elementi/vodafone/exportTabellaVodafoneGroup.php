...
$query = "SELECT creatoDa,round(sum(pesoTotaleLordo),5) as 'Peso Lordo' ,round(sum(pesoTotaleNetto),5)as 'Peso Netto',round(sum(pesoPagato),5)as 'Peso Pagato'  FROM `vodafone` inner join aggiuntaVodafone on vodafone.id=aggiuntaVodafone.id where mese='$meseSelezionato' group by creatoDa";
