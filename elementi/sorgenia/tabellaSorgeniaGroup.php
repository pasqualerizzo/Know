...
$queryMese = "SELECT mese FROM aggiuntaSorgenia group by mese ORDER by mese DESC";
...
$query = "SELECT creatoDa,round(sum(totalePesoLordo),5) as 'Peso Lordo' ,round(sum(pesoTotalePagato),5)as 'Peso Pagato'  FROM `sorgenia` inner join aggiuntaSorgenia on sorgenia.id=aggiuntaSorgenia.id where mese='$meseSelezionato' group by creatoDa";
...
<body>
<header>
    <h1 class="titolo">Sorgenia:<br>per Operatore</h1>
</header>
<div>
...
<script>
    ...
    function redirigi(sel) {
        ...
        window.location.href = "tabellaEnelGroup.php?meseSelezionato=" + data;
    ...
    function scaricaCSV() {
        ...
        window.location.href = "exportTabellaSorgeniaGroup.php?meseSelezionato=" + valore;
    }
</script>
</html>
