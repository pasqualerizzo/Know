...
$queryMese = "SELECT mese FROM aggiuntaVivigas group by mese ORDER by mese DESC";
...
$query = "SELECT creatoDa,round(sum(totalePesoLordo),5) as 'Peso Lordo' ,round(sum(pesoTotalePagato),5)as 'Peso Pagato' ,round(sum(pesoFormazione),5)as 'Peso Formazione' FROM `vivigas` inner join aggiuntaVivigas on vivigas.id=aggiuntaVivigas.id where mese='$meseSelezionato' group by creatoDa";
...
<script>
    ...
    function redirigi(sel) {
        ...
        window.location.href = "tabellaVivigasGroup.php?meseSelezionato=" + data;
