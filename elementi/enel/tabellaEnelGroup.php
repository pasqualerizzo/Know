...
$queryMese = "SELECT mese FROM aggiuntaEnel group by mese ORDER by mese DESC";
...
$query = "SELECT creatoDa,round(sum(totalePesoLordo),5) as 'Peso Lordo' ,round(sum(pesoTotalePagato),5)as 'Peso Pagato'  FROM `enel` inner join aggiuntaEnel on enel.id=aggiuntaEnel.id where mese='$meseSelezionato' group by creatoDa";
...
<body>
<header>
    <h1 class="titolo">Enel:<br>per Operatore</h1>
</header>
<div>
...
<script>
    ...
    function scaricaCSV() {
        ...
        window.location.href = "exportTabellaEnelGroup.php?meseSelezionato=" + valore;
    }
</script>
</html>
