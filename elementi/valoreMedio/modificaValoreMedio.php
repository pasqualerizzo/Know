...
$query = "SELECT * FROM `mediaPraticaMese` where id='$id'";
...
<html>
    ...
    <body>
        <header>
            <h1 class="titolo">Modifica Valore Medio</h1>
        </header>
        <div>
...
<?php include '/Applications/MAMP/htdocs/Know/elementi/sidebar.html' ?>
        <div class="pagina">
            <form action="queryModificaValoreMedio.php" method="POST">
                <fieldset>
                    <legend>Modifica Valore Medio</legend>
                    <label for="id">ID</label>
                    <input type="text" name="id" readonly value=<?=$riga[0]?>>
                    <br>
                    <label for="mandato">Mandato</label>
                    <input type="text" name="mandato" readonly value=<?=$riga[1]?>>
                    <br>
                    <label for="mese">Mese
                        <input type="text" name="mese" readonly value=<?=$riga[2]?>>
                    </label>
                    <label for="media">Media</label>
                    <input type="number" name="media"   min="0" max="500" step="0.001" value=<?=$riga[3]?>>
                    <br>
