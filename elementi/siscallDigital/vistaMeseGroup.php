...
$queryMese = "SELECT mese FROM `stringheSiscallDigital` group by mese";
...
$query = "SELECT user,nomeCompleto,sede,userGroup,mandato,SUM(pause),SUM(wait),SUM(talk),SUM(dispo),SUM(numero),Sum(dead) FROM `stringheSiscallDigital`where mese='$meseSelezionato' group by user";
...
<script>
    ...
    function scaricaCSV() {
        ...
        window.location.href = "exportVistaMeseGroup.php?meseSelezionato=" + valore;
    }
</script>
</html>
