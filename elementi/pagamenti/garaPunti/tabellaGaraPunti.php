...
$queryMese = "SELECT mese FROM `garaPunti` GROUP by mese ORDER by mese DESC";
...
$query = "SELECT * FROM `garaPunti` where mese='$meseSelezionato' order by  fascia";
...
<html>
    ...
    <body>
        <header>
            <h1 class="titolo">Pagamento:<br>Gara Punti</h1>
...
?>
    ...
    <script>
        ...
        function redirigi(sel) {
            ...
            window.location.href = "tabellaGaraPunti.php?meseSelezionato=" + data;

        }
        function aggiungiMese() {
            window.location.href = "aggiungiMeseGaraPunti.php";
        }

       document.addEventListener('DOMContentLoaded', function () {
    var cells = document.querySelectorAll('#pesi td');
    cells.forEach(function (cell) {
        cell.addEventListener('click', function () {
            var row = cell.closest('tr');
            var id = row.querySelector('td').innerHTML;
            window.location.href = 'modificaGaraPunti.php?id=' + id;
        });
    });
});
