...
//$ieri = date('Y-m-d', strtotime('2025-02-17'));
$provenienza = "siscallLead";
/**
...
$queryRicerca = " "
        ...
        . " Talk,"
        . " Dispo,"
        . " (Pause+Wait+Talk+Dispo) as numero,"
        . " Dead,"
        . " Data_Assunzione,"
        . " Nome_Cognome,"
        . " tipo_Pausa,"
        . " pausa626, "
        . " (pausa626 + Wait + Talk + Dispo) AS numero626 "
        . " FROM ("
        . " SELECT "
        . " v.user AS USER, "
        . "operatore.full_name AS 'Nome_Completo', "
        ...
        . "operatore.custom_one AS 'Data_Assunzione', "
        . " operatore.custom_three AS 'Nome_Cognome', "
        . " sub_status AS 'tipo_Pausa',"
        . " sum(if(sub_status='626',pause_sec,0)) as 'pausa626' "
        ...
        . "WHERE event_time >= '2025-02-01 00:00:00' "
...
if (!$risultato) {
    ...
    $queryTruncate = "TRUNCATE TABLE `stringheSiscallLead`";
    $conn19->query($queryTruncate);
    while ($riga = $risultato->fetch_array()) {
        $user = $riga['USER'];
        $nomeCompleto = $riga['Nome_Completo'];
        $livello = $riga['livello'];
        $sede = strtolower($riga['citta']);
        $giorno = date('Y-m-d', strtotime($riga['giorno']));
        $mese = $riga['mese'];
        $userGroup = $riga['user_group'];
        $mandato = $riga['mandato'];
        $pause = $riga['Pause'];
        $wait = $riga['Wait'];
        $talk = $riga['Talk'];
        $dispo = $riga['Dispo'];
        $numero = $riga['numero'];
        $dead = $riga['Dead'];
        $dataAssunzione = date('Y-m-d', strtotime($riga['Data_Assunzione']));
        $nomeCognome = $riga['Nome_Cognome'];
        $tipoPausa=$riga['tipo_Pausa'];
        $pausa626=$riga['pausa626'];
        $numero626=$riga['numero626'];
        ...
        $queryInserimento = "INSERT INTO `stringheSiscallLead`"
                . "(`user`, `nomeCompleto`, `livello`, `sede`, `giorno`, `mese`, `userGroup`, `mandato`, `pause`,"
                . " `wait`, `talk`, `dispo`, `numero`, `dead`, `dataAssunzione`, `NomeCognome`, `dataImport`, `provenienza`,idSede,idMandato,tipoPausa,pausa626,numero626) "
                . "VALUES ('$user','$nomeCompleto','$livello','$sede','$giorno','$mese','$userGroup','$mandato','$pause',"
                . " '$wait','$talk','$dispo','$numero','$dead','$dataAssunzione','$nomeCognome','$dataImport','$provenienza','$idSede','$idMandato','$tipoPausa','$pausa626','$numero626')";
