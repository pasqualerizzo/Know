...
if ($orario == "09") {
    ...
    while ($rigaSiscall = $risultatoSiscall->fetch_array()) {
        ...
        if (($risultato->num_rows) == 0) {
            ...
            $okTim = okTimOperatore($conn19, $dataMaggiore, $dataMinore, $operatore);
            $okTimOut = okOutTimOperatore($conn19, $dataMaggioreOre, $dataMinoreOre, $operatore);

            $ok = $okPleni + $okVivi + $okIren + $okUnion + $okVoda + $okEnel + $okTim;
            $okOut = $okPleniOut + $okViviOut + $okIrenOut + $okUnionOut + $okVodaOut + $okEnelOut + $okTimOut;
    ...
    $orarioMinore = $orario - 1;
    //$orario = "09";
    //$orarioMaggiore = "20";
    $dataOrarioMaggiore = date("Y-m-d " . $orario . ":00:00");
    echo $dataOrarioMaggiore . "<br>";

    $dataOrarioMinore = date("Y-m-d " . $orarioMinore . ":00:00");
    echo $dataOrarioMinore . "<br>";
    ...
    while ($rigaSiscall = $risultatoSiscall->fetch_array()) {
        ...
        $fullName = $rigaSiscall[0];
        $userGroup = $rigaSiscall[1];
        $livello = $rigaSiscall[2];
        $chiamateMassime = $rigaSiscall[3];
        $operatore = $rigaSiscall[4];
        $dataImport = date('Y-m-d H:i:s');
        $i++;
        $chiamateRicevute = chiamateRicevute($conn, $dataOrarioMinore, $dataOrarioMaggiore, $operatore);
//echo $operatore."<br>";
//        $chiamateMassime = $chiamateMassime + 3;
//        $queryUpdate = "UPDATE `vicidial_users` SET user_level='$livello',email='$dataImport',max_inbound_calls=$chiamateMassime  WHERE full_name='$fullName' ";
//         $conn->query($queryUpdate);

        switch ($livello) {
            case 1:
                if ($chiamateMassime < 12) {
                    if ($chiamateRicevute > 0) {
                        $chiamateMassime = $chiamateMassime + 4;
                        $queryUpdate = "UPDATE `vicidial_users` SET user_level='$livello',email='$dataImport',max_inbound_calls=$chiamateMassime  WHERE full_name='$fullName' ";
                        $conn->query($queryUpdate);
                    }
                }
            ...
            case 2:
                ...
                break;
            case 3:
                if ($chiamateMassime < 25) {
                    if ($chiamateRicevute > 0) {
                        $chiamateMassime = $chiamateMassime + 5;
                        $queryUpdate = "UPDATE `vicidial_users` SET user_level='$livello',email='$dataImport',max_inbound_calls=$chiamateMassime  WHERE full_name='$fullName' ";
                        $conn->query($queryUpdate);
                    }
                }
