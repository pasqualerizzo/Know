...
c.codicefiscalesocialetitolare AS codicefiscalesocialetitolare,
c.codicematricolaondapiu AS codicematricola,
...
c.ondapiu AS ondapiu,
c.ondapiuno AS ondapiuno,
...
c.metodpPagamento AS metododipagamento,
e.modifiedtime AS modifiedtime,
c.motivazionekoondapiu AS motivazioneko,
...
c.ondapiuid AS ondapiuid


FROM 
        vtiger_ondapiucf as cf 
        inner join vtiger_ondapiu as c on cf.ondapiuid=c.ondapiuid 
        inner join vtiger_crmentity as e on c.ondapiuid=e.crmid 
        inner join vtiger_users as operatore on e.smownerid=operatore.id 
        WHERE 
        c.datasottoscrizionecontratto >'2025-02-10' and  e.deleted=0 
        AND c.statopdaondapiu not in ('Annullata','Annullato')  
";
 ...
 */ else {
    truncateOndapiu($conn19);
    truncateAggiuntaOndapiu($conn19);
    /**
    ...
    while ($riga = $risultato->fetch_array()) {
         ...
         */
        try {
            $queryInserimento = "INSERT INTO `ondapiu`"
...
//            echo "----".$ore;
         ...
         */
        $queryInserimentoSecondario = "INSERT INTO `aggiuntaOndapiu"
