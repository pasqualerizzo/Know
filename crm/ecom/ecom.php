...
require "/Applications/MAMP/htdocs/Know/crm/ecom/funzioniEcom.php";
...
c.codicematricolaecom AS codicematricola,
...
c.motivazionekoecom AS motivazioneko,
...
FROM 
        vtiger_ecomcf as cf 
        inner join vtiger_ecom as c on cf.ecomid=c.ecomid 
        inner join vtiger_crmentity as e on c.ecomid=e.crmid 
        inner join vtiger_users as operatore on e.smownerid=operatore.id 
        WHERE 
        c.datasottoscrizionecontratto >'2025-02-10' and  e.deleted=0 
        AND c.statopdaecom not in ('Annullata','Annullato')  
";
 ...
 */ else {
    ...
    while ($riga = $risultato->fetch_array()) {
         ...
         */
        try {
            $queryInserimento = "INSERT INTO `ecom`"
...
//            echo "----".$ore;
         ...
         */
        $queryInserimentoSecondario = "INSERT INTO `aggiuntaEcom"
